...
bool function ArrayRemoveKeyWord(KeyWord[] akMyArray, KeyWord afMyKeyWord, bool abSort = false) global
    ;Removes a form from the array, if found. Sorts the array using ArraySortForm() if abSort is true.
    ...
    bool function ArraySortKeyWord(KeyWord[] akMyArray, int akI = 0) global
    ;Removes blank elements by shifting all elements down.
        ;-----------\
        ;Description \
        ;----------------------------------------------------------------
            ...
            if akMyArray[akI] == none
                ...
                if bFirstNoneFound == true
                    ...
                    if !(akMyArray[akI] == none)
                        ...
                        akMyArray[akI] = none
                        ;Call this function recursively until it returns
                        ArraySortKeyWord(akMyArray, iFirstNonePos + 1)
                        return true
                    else
                        akI += 1
                    endif
...
int function ArrayCountKeyWord(KeyWord[] akMyArray, KeyWord akMyKeyWord) global
    ;Attempts to count the number of times the given KeyWord appears in this array.
    ;-----------\
    ;Description \	Author: Chesko
    ;----------------------------------------------------------------
    ;Attempts to count the number of times the given KeyWord appears in this array.
    ...
    ;----------------------------------------------------------------
    ;		0					= 		KeyWord not found
    ;		int i		 		=		Number of times KeyWord appears in array
    int i = 0
    int iCount = 0
    while i < akMyArray.Length
        if akMyArray[i] == akMyKeyWord
            iCount += 1
            i += 1
        else
            i += 1
...
bool function ArrayAddActor(Actor[] akMyArray, Actor akMyActor) global
    ;Adds a form to the first available element in the array.
...
bool function ArrayRemoveActor(Actor[] akMyArray, Actor afMyActor, bool abSort = false) global
    ;Removes a form from the array, if found. Sorts the array using ArraySortForm() if abSort is true.
        ...
        int i = 0
        while i < akMyArray.Length
            if akMyArray[i] == afMyActor
                akMyArray[i] = none
                ;notification("Removing element " + i)
                if abSort == true
                    ArraySortActor(akMyArray)
                endif
                return true
            else
                i += 1
    ...
    bool function ArraySortActor(Actor[] akMyArray, int akI = 0) global
    ;Removes blank elements by shifting all elements down.
        ;-----------\
        ;Description \
        ;----------------------------------------------------------------
            ...
            if akMyArray[akI] == none
                ...
                if bFirstNoneFound == true
                    ...
                    if !(akMyArray[akI] == none)
                        ...
                        akMyArray[akI] = none
                        ;Call this function recursively until it returns
                        ArraySortActor(akMyArray, iFirstNonePos + 1)
                        return true
                    else
                        akI += 1
                    endif
...
function ArrayClearActor(Actor[] akMyArray) global
    ;Deletes the contents of this array.
    ;-----------\
    ;Description \
    ;----------------------------------------------------------------
...
int function ArrayHasActor(Actor[] akMyArray, Actor afMyActor) global
    ;Attempts to find the given form in the given array, and returns the index of the form if found.
    ;-----------\
    ;Description \	
    ;----------------------------------------------------------------
...
int function ArrayTotalCountActor(Actor[] akMyActor) global
    ;Counts the number of indices in this array that do not have a "none" type.
    ;-----------\
    ;Description \	Author: Chesko
    ;----------------------------------------------------------------
...
int function ArrayCountActor(Actor[] akMyArray, Actor akMyActor) global
    ;Attempts to count the number of times the given Actor appears in this array.
    ;-----------\
    ;Description \	Author: Chesko
    ;----------------------------------------------------------------
    ;Attempts to count the number of times the given Actor appears in this array.
    ...
    ;----------------------------------------------------------------
    ;		0					= 		Actor not found
    ;		int i		 		=		Number of times Actor appears in array
    int i = 0
    int iCount = 0
    while i < akMyArray.Length
        if akMyArray[i] == akMyActor
            iCount += 1
            i += 1
        else
            i += 1
...
bool function ArrayAddObjectReference(ObjectReference[] akMyArray, ObjectReference akMyObjectReference) global
    ;Adds a form to the first available element in the array.
    ...
    while i < akMyArray.Length
        if akMyArray[i] == none
            akMyArray[i] = akMyObjectReference
            ;notification("Adding " + akMyForm + " to the array.")
            return true
        else
            i += 1
...
bool function ArrayRemoveObjectReference(ObjectReference[] akMyArray, ObjectReference afMyObjectReference, bool abSort = false) global
    ;Removes a form from the array, if found. Sorts the array using ArraySortForm() if abSort is true.
        ...
        int i = 0
        while i < akMyArray.Length
            if akMyArray[i] == afMyObjectReference
                akMyArray[i] = none
                ;notification("Removing element " + i)
                if abSort == true
                    ArraySortObjectReference(akMyArray)
                endif
                return true
            else
                i += 1
    ...
    bool function ArraySortObjectReference(ObjectReference[] akMyArray, int akI = 0) global
    ;Removes blank elements by shifting all elements down.
        ;-----------\
        ;Description \
        ;----------------------------------------------------------------
            ...
            if akMyArray[akI] == none
                ...
                if bFirstNoneFound == true
                    ...
                    if !(akMyArray[akI] == none)
                        ...
                        akMyArray[akI] = none
                        ;Call this function recursively until it returns
                        ArraySortObjectReference(akMyArray, iFirstNonePos + 1)
                        return true
                    else
                        akI += 1
                    endif
...
function ArrayClearObjectReference(ObjectReference[] akMyArray) global
    ;Deletes the contents of this array.
    ;-----------\
    ;Description \
    ;----------------------------------------------------------------
...
int function ArrayHasObjectReference(ObjectReference[] akMyArray, ObjectReference afMyObjectReference) global
    ;Attempts to find the given form in the given array, and returns the index of the form if found.
    ;-----------\
    ;Description \	
    ;----------------------------------------------------------------
...
int function ArrayTotalCountObjectReference(ObjectReference[] akMyObjectReference) global
    ;Counts the number of indices in this array that do not have a "none" type.
    ;-----------\
    ;Description \	Author: Chesko
    ;----------------------------------------------------------------
    ...
    int i = 0
    int myCount = 0
    while i < akMyObjectReference.Length
        if akMyObjectReference[i] != none
            myCount += 1
            i += 1
        else
            i += 1
...
int function ArrayCountObjectReference(ObjectReference[] akMyArray, ObjectReference akMyObjectReference) global
    ;Attempts to count the number of times the given ObjectReference appears in this array.
    ;-----------\
    ;Description \	Author: Chesko
    ;----------------------------------------------------------------
    ;Attempts to count the number of times the given ObjectReference appears in this array.
    ...
    ;Return Values \
    ;----------------------------------------------------------------
    ;		0					= 		ObjectReference not found
    ;		int i		 		=		Number of times ObjectReference appears in array
    int i = 0
    int iCount = 0
    while i < akMyArray.Length
        if akMyArray[i] == akMyObjectReference
            iCount += 1
            i += 1
        else
            i += 1
